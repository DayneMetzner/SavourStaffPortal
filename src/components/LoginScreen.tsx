    ...
    try {
      ...
      if (result) {
        ...
        if (matchedProfile) {
          ...
          // They are signed in with Google, but have no staff profile yet!
          // Since they are fully authenticated, we can check for an active invitation in Firestore securely.
          let matchedInvitation: any = null;
          try {
            const { getInvitationFromFirestore } = await import('../utils/firestoreData');
            matchedInvitation = await getInvitationFromFirestore(authenticatedEmail);
          } catch (dbErr) {
            console.error("Login Screen: failed to pull invitation from Firestore:", dbErr);
          }

          if (matchedInvitation) {
            if (matchedInvitation.status === 'registered') {
              setLoginError(`The email "${authenticatedEmail}" has already been used to register. If you are unable to log in, please contact your coordinator.`);
              const { logoutFromGoogle } = await import('../utils/googleAuth');
              await logoutFromGoogle();
              return;
            }

            if (matchedInvitation.status === 'invited') {
              // Clear sign up form state and trigger onboarding
          ...
          // Fallback check to local cached state
          const localMatch = invitations.find(i => i.email.toLowerCase().trim() === authenticatedEmail);
          if (localMatch) {
            if (localMatch.status === 'registered') {
              setLoginError(`The email "${authenticatedEmail}" has already been used to register.`);
            } else {
              setShowSignUpForm(false);
              if (onStartOnboarding) {
                onStartOnboarding(authenticatedEmail);
          ...
          setLoginError(
            `Access Denied: No active staff profile or pending invitation found for Google Account "${authenticatedEmail}". Please ensure you are logging in with the correct email, or contact your coordinator to request an invitation.`
          );
          const { logoutFromGoogle } = await import('../utils/googleAuth');
          await logoutFromGoogle();
        }
            ...
            <div className="space-y-6">
              <div className="text-center space-y-2">
                <h2 className="text-sm font-bold text-slate-800 flex items-center justify-center gap-1.5">
                  <Sparkles size={16} className="text-indigo-600" /> Secure Staff Registration
                </h2>
                <p className="text-xs text-slate-500 leading-relaxed max-w-sm mx-auto">
                  To securely register your account and complete onboarding, you must authenticate using your Google Account.
                </p>
                <div className="p-3.5 bg-indigo-50/50 border border-indigo-100 rounded-xl text-[11px] leading-relaxed text-indigo-950 text-left space-y-2">
                  <p className="font-bold">How it works:</p>
                  <ul className="list-disc pl-4 space-y-1">
                    <li>Click the <strong>Sign In with Google to Register</strong> button below.</li>
                    <li>Choose the Google Account corresponding to the email where you received the invitation.</li>
                    <li>The system will automatically verify your invitation and launch the onboarding wizard!</li>
                  </ul>
                </div>
              </div>
              ...
              <button
                ...
                disabled={isLoading}
                className="w-full py-3.5 bg-indigo-600 hover:bg-indigo-500 text-white font-extrabold text-sm rounded-xl transition-all shadow-md shadow-indigo-600/10 cursor-pointer flex items-center justify-center gap-2.5 disabled:opacity-50"
              >
                {isLoading ? (
                ...
                {isLoading ? 'Connecting...' : 'Sign In with Google to Register'}
              </button>
